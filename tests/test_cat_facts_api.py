...
def test_get_multiple_cat_facts(
    ...
    params_str = f"limit={limit}, max_length={max_length}" if limit or max_length else "default"
    allure.dynamic.description(f"Validate the /facts endpoint with parameters: {params_str}")

    test_id = f"TC_CF_{3 if limit is not None else (4 if max_length is not None else 2)}"
